...
value = r'SIC'
resolution_n = r'50KM'
files = glob.glob(r'G:\icecon\micosoft\fitler\*.HDF')
files.sort()
latlon_file = r'E:\python_workfile\polar_project\FY3C_MWRI\lat_lon.h5'
save_path = r'G:\\polor_project\\output_new\\micosoft_save\\FY-3C\\all\\'
# 如果运行中断，从哪个文件开始继续运行
con_point = 0
# files =files[-350:]
...
fy_mwri = HaiYangData(satellite=satellite, sensor=sensor,resolution=50000)
...
for i,file in enumerate(files[con_point:]):
    ...
    with Dataset(save_path + file_name + 'all.h5', 'w') as f:
        f.createDimension('x', grid_array.shape[0])
        f.createDimension('y', grid_array.shape[1])
        # 添加数据属性
